	...
	if !found {
		if errNE {
			return nil, false, errs.ErrFull{
				...
				Desc: fmt.Sprintf("no order found with id %s", orderID),
			}
		}
		return nil, false, nil
	}
...
func (s *Service) Activate(ctx context.Context, obj *model.Key) (*model.Key, error) {
	...
	availableKey, err := s.repoDb.GetForActivate(ctx, *obj.ProductID)
	if err != nil {
	...
	if availableKey == nil {
		return nil, errs.ErrFull{
			Err:  errs.ObjectNotFound,
			Desc: "Не найдено доступных ключей для данного продукта",
			Fields: map[string]string{
