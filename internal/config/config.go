...
var Conf = struct {
	...
	ProviderHttpRoutesRaw string                       `env:"PROVIDER_HTTP_ROUTES"`
	ProviderHttpRoutes    map[string]map[string]string `env:"-"`
}{}
...
func init() {
	...
	Conf.ProviderHttpRoutes = make(map[string]map[string]string)
	if len(Conf.ProviderHttpRoutesRaw) > 0 {
		err := yaml.Unmarshal([]byte(Conf.ProviderHttpRoutesRaw), &Conf.ProviderHttpRoutes)
		if err != nil {
			panic(err)
		}
	}
