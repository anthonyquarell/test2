...
func New(uri, username, password, p12CertPath, p12Password, serverCACertPath string) *Repo {
	tlsConfig := tlsConnection(p12CertPath, p12Password, serverCACertPath)
	return &Repo{
...
func (r *Repo) CreateOrder(ctx context.Context, obj *providerModel.OrderRequest) (*providerModel.OrderResponse, error) {
	apiReq := repoModel.EncodeActivateRequest(obj, uuid.New().String())
	...
	if err != nil {
		return nil, fmt.Errorf("failed to send ASBIS request: %w", err)
	}

	result := repoModel.DecodeActivateResponse(*apiResp)

	if apiResp.ErrorCode == "79004" {
		return nil, fmt.Errorf(string(repBody))
	}

	slog.Info("Parsed response from asbis", "parsed_response", apiResp)

	slog.Info("Final decoded response", "decoded", result)

	return result, nil
}

func (r *Repo) CancelOrder(ctx context.Context, obj *providerModel.CancelRequest) (*providerModel.CancelResponse, error) {
	apiReq := repoModel.EncodeCancelRequest(obj, uuid.New().String()) // генерируется новый ID аннулирования
	apiResp := &repoModel.OrderRep{}
	...
	slog.Info("Raw response from asbis", "response_body", string(repBody))
	slog.Info("Final decoded response", "decoded", result)
...
func tlsConnection(p12CertPath, p12Password, serverCACertPath string) *tls.Config {
	p12Data, err := os.ReadFile(p12CertPath)
	if err != nil {
		panic(fmt.Sprintf("failing load p12 certficiate: %v", err))
	}

	blocks, err := pkcs12.ToPEM(p12Data, p12Password)
	if err != nil {
		panic(fmt.Sprintf("failing decode p12 certficiate: %v", err))
	}
	...
	for _, b := range blocks {
		...
		case "PRIVATE KEY":
			...
			if err != nil {
				panic(fmt.Sprintf("failing parse private key: %v", err))
			}
	...
	if clientCert.PrivateKey == nil || len(clientCert.Certificate) == 0 {
		panic("missing private key or certificate in p12 file")
	}

	systemRoots, _ := x509.SystemCertPool()
	if systemRoots == nil {
		panic("missing systems root certificate in p12 file")
	}

	serverCACert, err := os.ReadFile(serverCACertPath)
	if err != nil {
		panic(fmt.Sprintf("failing load server certficiate: %v", err))
	}
	if !systemRoots.AppendCertsFromPEM(serverCACert) {
		panic(fmt.Sprintf("failing append server certificate: %v", err))
	}
