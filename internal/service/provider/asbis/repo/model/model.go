...
func EncodeActivateRequest(req *providerModel.OrderRequest, transactionId string) *OrderReq {
	return &OrderReq{
...
func EncodeCancelRequest(req *providerModel.CancelRequest, newTransactionId string) *OrderReq {
	return &OrderReq{
		...
		RefClientTransactionId: lo.FromPtr(req.ID),
...
func DecodeCancelResponse(rep OrderRep) *providerModel.CancelResponse {
	return &providerModel.CancelResponse{
		...
		ID:      lo.ToPtr(rep.ClientTransactionId),
	}
}
