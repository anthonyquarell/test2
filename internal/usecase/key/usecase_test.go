...
func TestUsecase_Activate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			customerPhone: "+77001112233",
			setupMock: func(ut *usecaseTest) {
				// Mock MDM product found (line 89)
				mdmProduct := &mdmModel.Product{
					ProviderID:        "provider-1",
					ProductID:         "prod-2",
			...
			expectedKey: nil,
			expectedErr: errors.New("Услуга провайдера в данный момент недоступна"),
		},
		{
...
func TestUsecase_Cancel(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(ut *usecaseTest) {
				ut.service.On("GetByOrderID", mock.Anything, "non-existent", true).Return(nil, false, nil).Once()
			},
			expectedID:  nil,
			expectedErr: nil,
		},
