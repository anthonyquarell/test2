...
type Usecase struct {
	...
	providerServiceMng ProviderServiceMngI
}

func New(service KeyServiceI, mdmService MdmServiceI, providerServiceMng ProviderServiceMngI) *Usecase {
	return &Usecase{
		service:            service,
		mdmService:         mdmService,
		providerServiceMng: providerServiceMng,
	}
}
...
func (u *Usecase) ActivateByProvider(ctx context.Context, productID, customerPhone string) (*Key, error) {
	...
	providerService, err := u.providerServiceMng.GetProvider(*product.ProviderID)
	if err != nil {
