...
func (u *Usecase) load(ctx context.Context, obj *model.Edit) error {
	...
	// обновляем ключ, если уже имеется запись в БД
	if existingKey != nil && existingKey.ProductID == *obj.ProductID {
		obj.ID = &existingKey.ID
		obj.Status = lo.ToPtr(constant.KeyStatusNew)
		obj.UpdatedAt = lo.ToPtr(time.Now())
		obj.OrderID = lo.ToPtr("")
		obj.CustomerPhone = lo.ToPtr("")
		obj.ProviderOrderID = lo.ToPtr("")
		obj.ProviderTransactionID = lo.ToPtr("")
		err = u.service.Update(ctx, obj)
		if err != nil {
...
func (u *Usecase) Activate(ctx context.Context, productID, orderID, customerPhone string) (*model.Main, error) {
	...
	if err != nil {
		...
		slog.Error("providerService.CreateOrder", "error", err)

		// Произошла ошибка провайдера - обращение к пулу
		poolErr := u.activate(ctx, nil, orderID, customerPhone, productID)
		if poolErr != nil {
			return nil, fmt.Errorf("activate: %w ", poolErr)
		}

		key, _, err := u.service.GetByOrderID(ctx, orderID, true)
		if err != nil {
	...
	// если createOrder прошел успешно - заполняем данные клиента
	err = u.activate(ctx, &id, orderID, customerPhone, productID)
	if err != nil {
...
func (u *Usecase) activate(ctx context.Context, id *string, orderID, customerPhone, productID string) error {
	...
	items, tCount, err := u.service.List(ctx, listReq)
	if err != nil {
		return fmt.Errorf("serivce.List: %w", err)
	}
