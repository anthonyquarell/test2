...
import (
	...
	"errors"
	"fmt"
	clientCptP "github.com/mechta-market/e-product/internal/client/comportal"
...
func (a *App) Init() {
	...
	var cptClient clientCptP.Client
	...
	var providerMng *servicePrvP.ServiceManager
	...
	// http client connections
	{
...

		{
			...
			for dbName, dbConfig := range config.Conf.ProviderHttpRoutes {
				comportalHttpClient.AddRoute(dbName, dbConfig["url"], dbConfig["username"], dbConfig["password"])
				slog.Info("Comportal route added",
					"db_name", dbName,
					"url", dbConfig["url"])
			}
			cptClient = comportalHttpClient
		}
	...
	// comportal
	{
		...
		repo = serviceCptRepoP.New(cptClient)
		cptService = serviceCptP.New(repo)
	}

	{
		providerMng = servicePrvP.NewManager()

		if cptService != nil {
			err := providerMng.RegisterProvider(constant.ProviderComportal, cptService)
			if err != nil {
				errCheck(err, "failed to register comportal provider")
			}
	...
	// key
	{
		...
		keyUsecase = usecaseKeyP.New(keyService, mdmService, providerMng)
...
func (a *App) initProviders(httpClients map[string]clientCptP.Client) *servicePrvP.ServiceManager {
	manager := servicePrvP.NewManager()

	if comportalClient, exists := httpClients["comportal"]; exists {
		slog.Info("Registering Comportal provider...")

		comportalRepo := serviceCptRepoP.New(comportalClient)
		comportalProvider := serviceCptP.New(comportalRepo)

		err := manager.RegisterProvider(constant.ProviderComportal, comportalProvider)
		if err != nil {
			panic(fmt.Sprintf("failed to register comportal provider: %v", err))
		}

		slog.Info("Comportal provider registered successfully",
			"provider_id", constant.ProviderComportal)
	} else {
		slog.Warn("Comportal client not configured, skipping provider registration")
	}

	slog.Info("Providers initialized",
		"total_providers", len(manager.ListProviders()))
